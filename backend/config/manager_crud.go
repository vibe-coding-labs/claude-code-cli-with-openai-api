...
func (cm *ConfigManager) UpdateConfig(id string, req *models.APIConfigRequest) (*models.APIConfig, error) {
	...
	if req.RetryCount > 0 {
		config.RetryCount = req.RetryCount
	}
	if req.RetryBackoffBase > 0 {
		config.RetryBackoffBase = req.RetryBackoffBase
	}
	if req.RetryBackoffMax > 0 {
		config.RetryBackoffMax = req.RetryBackoffMax
	}
