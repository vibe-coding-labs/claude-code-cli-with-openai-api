...
func (sh *SessionHandler) LoadConversationHistory(sessionID string, messages []models.ClaudeMessage) ([]models.ClaudeMessage, error) {
	...
	for _, msg := range historyMessages {
		...
		// If assistant message has tool_calls, merge them into content blocks
		if msg.Role == "assistant" && len(msg.ToolCalls) > 0 {
			var contentBlocks []interface{}

			// Try to parse existing content as JSON blocks
			if msg.Content != "" {
				var existingBlocks []interface{}
				if err := json.Unmarshal([]byte(msg.Content), &existingBlocks); err == nil {
					// Content is already a JSON array, use it directly
					contentBlocks = existingBlocks
				} else {
					// Content is plain text, add as text block
...
func (sh *SessionHandler) SaveMessages(sessionID string, messages []models.ClaudeMessage, usage *models.ClaudeUsage) error {
	...
	for i, msg := range messages {
		...
		if msg.Role == "assistant" {
			...
			// Check if content contains tool calls ([]interface{} type)
			...
			// Check if content contains tool calls ([]map[string]interface{} type)
			if blocks, ok := msg.Content.([]map[string]interface{}); ok {
