...
var testResourceCounter uint64
...
func createTestLoadBalancer(t *testing.T) *database.LoadBalancer {
	suffix := atomic.AddUint64(&testResourceCounter, 1)
	config1ID := fmt.Sprintf("test-config-1-%d", suffix)
	config2ID := fmt.Sprintf("test-config-2-%d", suffix)
	lbID := fmt.Sprintf("test-lb-%d", suffix)

	// Create test configs
	config1 := &database.APIConfig{
		ID:            config1ID,
		Name:          fmt.Sprintf("Test Config 1 %d", suffix),
		OpenAIBaseURL: "https://api.test1.com",
		Enabled:       true,
	}
	config2 := &database.APIConfig{
		ID:            config2ID,
		Name:          fmt.Sprintf("Test Config 2 %d", suffix),
	...
	lb := &database.LoadBalancer{
		ID:       lbID,
		Name:     fmt.Sprintf("Test Load Balancer %d", suffix),
