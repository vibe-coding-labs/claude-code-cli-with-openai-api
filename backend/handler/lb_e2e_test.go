...
func TestE2E_HealthCheckFlow(t *testing.T) {
	...
	// Node should become unhealthy after initial failures.
	assert.Eventually(t, func() bool {
		status, err := checker.GetHealthStatus(config.ID)
		if err != nil {
			return false
		}
		return status.Status == "unhealthy" && status.ConsecutiveFailures >= 2
	...
	// Then it should recover once backend starts succeeding.
	assert.Eventually(t, func() bool {
		status, err := checker.GetHealthStatus(config.ID)
		if err != nil {
			return false
		}
		return status.Status == "healthy" && status.ConsecutiveSuccesses >= 2
