...
func (g *GeminiConverter) ParseRequest(body []byte) (*InternalRequest, error) {
	...
	if geminiReq.GenerationConfig != nil {
		...
		// Extract response format (one-api pattern)
		if geminiReq.GenerationConfig.ResponseMimeType != "" {
			req.ResponseFormat = map[string]interface{}{
				"type": geminiReq.GenerationConfig.ResponseMimeType,
			}
		}
	}
	...
	for _, content := range geminiReq.Contents {
		...
		// Skip dummy model "Okay" messages (one-api pattern)
		if content.Role == "model" && len(content.Parts) == 1 && content.Parts[0].Text == "Okay" {
			continue
		}
...
func (g *GeminiConverter) BuildRequest(req *InternalRequest) ([]byte, error) {
	...
	// Add default safety settings (one-api pattern: prevent safety-related blocking)
	...
	// one-api pattern: if system is set as user message (no systemInstruction support),
	// add dummy model "Okay" response after it. For our case, we use systemInstruction
	// which is supported by modern Gemini models, so this is handled in ParseRequest.
	...
	if req.Temperature != nil || req.TopP != nil || req.TopK != nil || req.MaxTokens > 0 || len(req.StopSeqs) > 0 {
	...
	// Handle response format (one-api pattern: JSON schema / MIME type mapping)
	if req.ResponseFormat != nil {
		if rf, ok := req.ResponseFormat.(map[string]interface{}); ok {
			if geminiReq.GenerationConfig == nil {
				geminiReq.GenerationConfig = &models.GeminiGenerationConfig{}
			}
			if rfType, ok := rf["type"].(string); ok {
				if mimeType, ok := geminiResponseFormatMimeType[rfType]; ok {
					geminiReq.GenerationConfig.ResponseMimeType = mimeType
				}
			}
			if jsonSchema, ok := rf["json_schema"].(map[string]interface{}); ok {
				if schema, ok := jsonSchema["schema"].(map[string]interface{}); ok {
					geminiReq.GenerationConfig.ResponseSchema = stripUnsupportedSchemaFields(schema)
				}
			}
		}
	}
...
// defaultGeminiSafetySettings returns safety settings that minimize false positive blocking.
// Ported from one-api's default safety configuration for Gemini.
func defaultGeminiSafetySettings() []models.GeminiSafetySetting {
	return []models.GeminiSafetySetting{
...
// addDummyModelMessage appends a model "Okay" response after a user message,
// required by some Gemini models that don't support system instruction natively.
// Ported from one-api's shouldAddDummyModelMessage pattern.
func addDummyModelMessage(contents *[]models.GeminiContent) {
...
// geminiResponseFormatMimeType maps response format types to Gemini MIME types.
// Ported from one-api's mimeTypeMap.
var geminiResponseFormatMimeType = map[string]string{
