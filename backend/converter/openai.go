...
func (o *OpenAIConverter) BuildRequest(req *InternalRequest) ([]byte, error) {
	...
	// Log message roles for protocol debugging
	roles := make([]string, 0, len(openAIReq.Messages))
	for _, m := range openAIReq.Messages {
		role := m.Role
		if m.ToolCallID != "" {
			role += "(tool_call_id=" + m.ToolCallID + ")"
		}
		if len(m.ToolCalls) > 0 {
			role += fmt.Sprintf("(tool_calls=%d)", len(m.ToolCalls))
		}
		roles = append(roles, role)
	}
	utils.GetLogger().Debug("[BuildRequest] model=%s messages=%v tools=%d", openAIReq.Model, roles, len(openAIReq.Tools))
...
func (o *OpenAIConverter) validateAndFixMessageSequence(messages []models.OpenAIMessage) []models.OpenAIMessage {
	...
	for _, msg := range messages {
		...
		// Ensure non-tool messages have non-nil content
	...
	for i, msg := range fixed {
		...
		// After adding assistant with tool_calls, check if next messages provide all responses
		if msg.Role == "assistant" && len(msg.ToolCalls) > 0 {
			...
			// Scan ahead to find which tool_call_ids already have responses
			for j := i + 1; j < len(fixed); j++ {
				if fixed[j].Role == "tool" {
					if _, exists := expectedToolIDs[fixed[j].ToolCallID]; exists {
						expectedToolIDs[fixed[j].ToolCallID] = true
					}
				} else {
					break
				}
	...
	if len(msg.Content) == 1 && msg.Content[0].Type == "text" {
		...
		for _, cb := range msg.Content {
			...
			case "tool_result":
				...
				// 这里直接跳过，因为每个 tool_result 需要成为单独的 OpenAI tool 消息
				continue
			}
		}
		...
		if hasToolUse {
			...
			if len(thinkingParts) > 0 {
				// Thinking-only: inject placeholder so strict providers don't reject
...
func (o *OpenAIConverter) ParseResponse(body []byte) (*InternalResponse, error) {
	...
	// Map reasoning_content to thinking block (DeepSeek/o1 style)
	if message.ReasoningContent != "" {
		thinkingBlock := ContentBlock{Type: "thinking", Text: message.ReasoningContent}
		resp.Content = append([]ContentBlock{thinkingBlock}, resp.Content...)
	}

	// Map reasoning_details array to thinking blocks (GPT-5.x/o1-pro format)
	if details, ok := message.ReasoningDetails.([]interface{}); ok {
		for _, detail := range details {
			if dm, ok := detail.(map[string]interface{}); ok {
				if text, ok := dm["text"].(string); ok && text != "" {
					resp.Content = append([]ContentBlock{{Type: "thinking", Text: text}}, resp.Content...)
				} else if summary, ok := dm["summary"].(string); ok && summary != "" {
					resp.Content = append([]ContentBlock{{Type: "thinking", Text: summary}}, resp.Content...)
				}
			}
		}
	}
...
func (o *OpenAIConverter) BuildStreamEvent(event *StreamEvent) ([]byte, error) {
	...
	if event.Delta != nil {
		...
		if event.Delta.StopReason != "" {
			...
			switch finishReason {
			case "end_turn", "stop_sequence", "content_filter":
				finishReason = "stop"
			case "max_tokens":
				finishReason = "length"
			case "tool_use":
...
// maxToolNameLength is the maximum allowed tool name length for strict providers
