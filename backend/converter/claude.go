...
func (c *ClaudeConverter) ParseRequest(body []byte) (*InternalRequest, error) {
	...
	if claudeReq.Metadata != nil {
		if claudeReq.Metadata.UserID != "" {
			req.Metadata["user_id"] = claudeReq.Metadata.UserID
			if extractedSessionID := extractSessionIDFromMetadataUserID(claudeReq.Metadata.UserID); extractedSessionID != "" {
				req.Metadata["session_id"] = extractedSessionID
			}
		}
		if claudeReq.Metadata.SessionID != "" {
...
func (c *ClaudeConverter) ParseResponse(body []byte) (*InternalResponse, error) {
	...
	if err := validateClaudeMessageResponse(&claudeResp); err != nil {
		return nil, err
	}
...
func validateClaudeMessageResponse(resp *models.ClaudeResponse) error {
	if resp == nil {
		return fmt.Errorf("invalid claude response: response is nil")
	}
	if resp.Type != "" && resp.Type != "message" {
		return fmt.Errorf("invalid claude response: unexpected type %q", resp.Type)
	}
	if resp.Role == "" {
		return fmt.Errorf("invalid claude response: missing role")
	}
	if resp.StopReason == "" {
		return fmt.Errorf("invalid claude response: missing stop_reason")
	}
	if len(resp.Content) == 0 {
		return fmt.Errorf("invalid claude response: empty content")
	}
	return nil
}
