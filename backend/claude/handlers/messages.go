...
// formatContent formats message content for logging
...
func (h *MessagesHandler) CreateMessage(c *gin.Context) {
	...
	for i, msg := range req.Messages {
		fmt.Printf("   Message[%d]: role=%s, content=%s\n", i, msg.Role, formatContent(msg.Content))
	}
	fmt.Printf("   Max tokens: %d\n", req.MaxTokens)
	fmt.Printf("   Stream: %v\n", req.Stream)
	fmt.Printf("   Tools: %d\n", len(req.Tools))
	for i, tool := range req.Tools {
		fmt.Printf("   Tool[%d]: name=%s, type=%s\n", i, tool.Name, tool.Type)
	}
...
// 用于处理工具调用+流式请求的场景
func (h *MessagesHandler) handleNonStreamAsStream(c *gin.Context, openAIReq *oldModels.OpenAIRequest, claudeReq *models.MessagesRequest) {
	// 执行非流式请求
	openAIResp, err := h.client.CreateChatCompletion(openAIReq)
	if err != nil {
		fmt.Printf("❌ OpenAI API error: %v\n", err)
	...
	sendSSEEvent(c, "message_start", msgStart)

	// 发送 ping
	sendSSEEvent(c, "ping", map[string]string{"type": "ping"})
	...
	for i, block := range claudeResp.Content {
		...
		case "text":
			...
			sendSSEEvent(c, "content_block_start", startEvt)
			...
			sendSSEEvent(c, "content_block_delta", deltaEvt)
			...
			sendSSEEvent(c, "content_block_stop", stopEvt)
		...
		case "tool_use":
			...
			// content_block_delta - 发送完整的input
			deltaEvt := map[string]interface{}{
				"type":  "content_block_delta",
				"index": idx,
				"delta": map[string]interface{}{
	...
	sendSSEEvent(c, "message_delta", msgDelta)

	// 发送 message_stop
	sendSSEEvent(c, "message_stop", map[string]string{"type": "message_stop"})
...
func (h *MessagesHandler) handleStreamingResponse(c *gin.Context, openAIReq *oldModels.OpenAIRequest, claudeReq *models.MessagesRequest) {
	...
	// Use Go converter for streaming (litellm bridge removed)
	oldReq := convertToOldFormat(claudeReq)
