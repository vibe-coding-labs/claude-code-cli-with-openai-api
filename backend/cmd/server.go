...
func runServer(cmd *cobra.Command, args []string) error {
	...
	// Import CCR (Claude Code Router) providers
	if imported, err := importCCRConfig(); err != nil {
		color.New(color.FgYellow, color.Bold).Print("⚠️  CCR Import: ")
		color.New(color.FgYellow).Printf("%v\n", err)
	} else if imported > 0 {
		color.New(color.FgGreen, color.Bold).Print("✅ CCR Import: ")
		color.New(color.FgGreen).Printf("Imported %d provider(s) from Claude Code Router\n", imported)
	}
	...
	authAPI := router.Group("/api/auth")
	{
		...
		authAPI.GET("/server-info", h.ServerInfo)
	}
	...
	configAPI.Use(handler.AuthMiddleware())
	{
		...
		// API Key management
		configAPI.GET("/admin/tenants/:id/api-keys", adminAPIHandler.ListAPIKeys)
		configAPI.POST("/admin/tenants/:id/api-keys", adminAPIHandler.CreateAPIKey)
		...
		// Quota management
		configAPI.GET("/admin/tenants/:id/quota", adminAPIHandler.GetTenantQuota)
		configAPI.POST("/admin/tenants/:id/quota", adminAPIHandler.SetTenantQuota)
		configAPI.POST("/admin/tenants/:id/quota/:quota_type/reset", adminAPIHandler.ResetTenantQuota)

		// Rate limit management
		configAPI.GET("/admin/tenants/:id/rate-limits", adminAPIHandler.ListRateLimits)
		configAPI.POST("/admin/tenants/:id/rate-limits", adminAPIHandler.CreateRateLimit)
		configAPI.DELETE("/admin/rate-limits/:limit_id", adminAPIHandler.DeleteRateLimit)

		// IP rule management
		configAPI.GET("/admin/tenants/:id/ip-rules", adminAPIHandler.ListIPRules)
		configAPI.POST("/admin/tenants/:id/ip-rules", adminAPIHandler.CreateIPRule)
		configAPI.DELETE("/admin/ip-rules/:rule_id", adminAPIHandler.DeleteIPRule)

		// Usage and billing
		configAPI.GET("/admin/tenants/:id/usage", adminAPIHandler.GetTenantUsage)
		configAPI.POST("/admin/tenants/:id/billing-report", adminAPIHandler.GenerateBillingReport)
