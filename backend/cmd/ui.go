...
func runUI(cmd *cobra.Command, args []string) error {
	...
	cfg := &config.Config{
		...
		MaxTokensLimit:  getEnvAsInt("MAX_TOKENS_LIMIT", 16384),
	...
	if cfg.LogLevel == "DEBUG" {
		...
		// Initialize database (same as server command)
	...
	h := handler.NewHandler(cfg)
		...
		// Setup API routes (same as server command)
		router.GET("/", h.Root)
		router.GET("/health", h.HealthCheck)
		router.GET("/test-connection", h.TestConnection)

		// Standard Claude API routes
		v1 := router.Group("/v1")
		{
			...
			v1.POST("/configs/:id/messages", h.CreateMessageWithConfig)
