...
// ErrorWithCause logs an error with its root cause chain for debugging
func (l *Logger) ErrorWithCause(err error, format string, args ...interface{}) {
	message := fmt.Sprintf(format, args...)
	if err != nil {
...
// LogUpstreamError logs a detailed upstream API error with full context
func (l *Logger) LogUpstreamError(provider, endpoint, model string, statusCode int, responseBody string) {
	// Truncate response body for readability
	bodyPreview := responseBody
	if len(bodyPreview) > 500 {
		bodyPreview = bodyPreview[:500] + "...(truncated)"
	}
	l.log(ERROR, "UPSTREAM ERROR: provider=%s endpoint=%s model=%s status=%d | response: %s",
		provider, endpoint, model, statusCode, bodyPreview)
}

// LogRequestError logs a request handling error with full context
func (l *Logger) LogRequestError(requestID, endpoint, model string, err error, attempt, maxAttempts int) {
	l.log(ERROR, "REQUEST FAILED: req=%s endpoint=%s model=%s attempt=%d/%d | error: %v",
		requestID, endpoint, model, attempt, maxAttempts, err)
}
