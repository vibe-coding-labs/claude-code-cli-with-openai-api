...
const ConfigTestInline: React.FC<ConfigTestInlineProps> = ({ configId }) => {
  ...
  const handleTest = async () => {
    ...
    try {
      const response = await axios.post(`/api/configs/${configId}/test`, {
        message: userMessage,
      });
    ...
    <div>
      <Card>
        <div style={{ marginBottom: 16 }}>
          <Paragraph>
            在此测试配置是否正常工作。系统会发送一条测试消息到OpenAI API，并返回响应结果。
          </Paragraph>
        </div>
...
export default ConfigTestInline;
